...
\begin{center}
\note{On part de notre objet impossible}
\includegraphics<1>[scale=.4]{../Images/tribar.pdf}
\note{Si on ne regarde que le coin en haut à gauche ...}
\includegraphics<2>[scale=.4]{../Images/UpperCorner.pdf}
\note{... on remarque qu'il n'a plus rien d'impossible !}
\includegraphics<3>[scale=.4]{../Images/UpperCorner2.pdf}
\note{On peut le réaliser en vrai avec deux bouts de bois et un peu de colle}
\includegraphics<4>[scale=.4]{../Images/UpperCorner3.pdf}
\end{center}
\end{frame}
...
\begin{center}
\note{Revenons à notre tribarre ou plutôt son dessin}
\includegraphics<1>[scale=.4]{../Images/tribar.pdf}
\note{On peut découper ce dessin en trois parties autour de chaque coin.}
\includegraphics<2>[scale=.4]{../Images/Covering.pdf}
\note{...parties qui s'intersectent}
...
\begin{frame}% Split
\begin{center}
\note{Éclatons donc notre dessin. On a trois dessins qui chacun représente des objets RÉALISABLES}
\includegraphics<1>[scale=.4]{../Images/Covering_split.pdf}
\note{Ces trois dessins doivent être recollés suivant les zones rouges pour montrer obtenir le dessin d'origine.}
\note{Notre figure impossible est LOCALEMENT possible. Mais si les trois dessins peuvent se recoller pour donner le dessin de la tribarre, les trois objets physiques eux ne peuvent pas ! C'est ce qu'on va voir.}
\end{center}
\end{frame}
...
\begin{center}
\note{Appelons $U_1$, $U_2$ et $U_3$ les trois parties qui recouvrent le dessin. Prenons un point $A$ sur le dessin qui soit à la fois dans $U_1$ et $U_2$}
\includegraphics<1>[scale=.4]{../Images/A.pdf}
\note{Après découpage, le point $A$ se dédouble : une copie $A_{1}$ dans $U_1$ et une copie $A_{2}$ dans $U_2$}
...
\includegraphics<2>[scale=.4]{../Images/1Loin2Pres_.pdf}
\end{center}
\end{frame}
...
\begin{frame}% Dij
\note{
Pour garder cette information en mémoire, on va noter
}
\begin{equation*}
...
À quelle(s) condition(s) les trois objets construits se recollent-t-ils ? \pause

\bigskip
Il faut
\medskip
\begin{itemize}[<+->]
\item que $A_1$ et $A_2$ se superposent \uncover<5->{ : \quad  $d_{12} = 1$}
...
\hspace*{-8em}\includegraphics<2>[width=1.8 \paperwidth]{../Images/Inception2.jpg}
\end{center}
\end{frame}
...
\note{C'est l'information importante sur cette construction.}
\end{center}
\end{frame}
...
\begin{frame}% Homothétie 2
\begin{center}
Il existe une manière de redimensionner les trois objets de tel sorte que $d_{12} = d_{13} = d_{23} = 1$ \pause\\
( et donc de recoller les trois object en une tribarre )\\
\bigskip\pause
si et seulement si\\
\bigskip\pause
\begin{equation*}
...
\begin{frame}
\begin{center}
\note{Si l'on regarde l'ensemble des triplet $d_{12},d_{13}, d_{23}$ mais qu'on force ceux qui s'écrivent comme des quotients de $\lambda_i$ à s'annuler on a construit un objet de ce qu'on appelle \textbf{espace de cohomologie}}
\[
...
\]}
\includegraphics<4>[scale=.4]{../Images/B3<C3.pdf}
\only<5,7>{
\[
\text{distance}(B_3) < \text{distance}(C_3)
\]}
\includegraphics<6>[scale=.4]{../Images/A2>C2.pdf}
\only<7>{
\[
...
\begin{center}
\begin{align*}
d_{12} \times d_{23} &= \dfrac{\text{distance}(A_1)}{\text{distance}(A_2)} \times \dfrac{\text{distance}(C_2)}{\text{distance}(C_3)}\\
\uncover<2->{&< \dfrac{\text{distance}(A_1)}{\text{distance}(C_2)} \times \dfrac{\text{distance}(C_2)}{\text{distance}(C_3)}\\ }
\uncover<3->{&< \dfrac{\text{distance}(A_1)}{\text{distance}(C_3)}\\  }
\uncover<4->{&< \dfrac{\text{distance}(B_1)}{\text{distance}(C_3)}\\  }
\uncover<5->{&< \dfrac{\text{distance}(B_1)}{\text{distance}(B_3)}\\  }
\uncover<6->{&< d_{13} }\\
\end{align*}
\pause
La tribarre n'existe pas.
\end{center}
\end{frame}
...
\begin{frame}
\begin{center}
\note{L'intérêt n'est pas de montrer que la tribarre est impossible ! L'intérêt c'est qu'en mathématique (en algèbre et en géométrie), quand quelque chose ne marche pas, eh bien la vie ne s'arrête pas. Il y a des choses qui empêchent que ça marche et l'étude de ces obstructions se révèle bien souvent très riche}\vspace*{-2.5em}
